...
int GDI_GetBoldNumHeight(int size) {
	...
	result = (int)(offset/ceil((double)width/8));
	return result;
}
...
void GDI_PaintBoldNum(int x, int y, char * str, int alignment, int highlight, int fontsize) {
	...
	for(i = 0; i < strlen(str); i++) {
		...
		if(-1 == index) {
			...
			wsum = GetBoldNumWidth(-1, fontsize);
		}
		//height = (int)((GetBoldNumOffset(index+1, fontsize) - GetBoldNumOffset(index, fontsize))/ceil((double)wsum/8));
